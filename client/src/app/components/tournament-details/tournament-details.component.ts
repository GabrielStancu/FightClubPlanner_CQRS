...
import { NotTestedFighter } from 'src/app/shared/helpers/not-tested-fighter.model';
...
export class TournamentDetailsComponent implements OnInit {
  ...
  refreshTournamentData(): void{
    this.managerService.getTournamentInfo(this.tournamentId).subscribe(tournamentData => {
      ...
      this.fighterDecorators = [];
      tournamentData.fighters.forEach(f => {
        if (f.testsCount === 0){
          this.fighterDecorators.push(new NotTestedFighter(f));
        } else {
          if ( f.isEligible === true){
            this.fighterDecorators.push(new HealthyFighter(f));
          } else {
            this.fighterDecorators.push(new SickFighter(f))
          }
        }
      });

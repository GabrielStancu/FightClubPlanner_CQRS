...
namespace FightClubPlannerTests
{
    public class FighterControllerTest
    {
        ...
        private InviteAnsweredHandler _inviteAnsweredHandler;
        private TestRegisteredHandler _testRegisteredHandler;
        private DetailedFighterHandler _detailedFighterHandler;
        ...
        public void AnsweredInviteIsUpdated()
        {
            ...
            _inviteAnsweredHandler.HandleRequestAsync(_mapper.Map<Invite, InviteAnswered>(invite)).Wait();
        ...
        public void RegisteredTestIsSaved()
        {
            ...
            var test = new TestRegistered()
            {
                FighterId = fighter.Id,
                IsPositive = true,
                IsolationBubbleId = isolationBubble.Id
            };
            _testRegisteredHandler.HandleRequestAsync(test).Wait();
            var tests = _context.CovidTest.Where(ct => ct.FighterId == fighter.Id).ToList();
            var registeredTest = tests.ElementAt(tests.Count - 2);
        ...
        public void FighterDetailsAreRetrieved()
        {
            ...
            var detailedFighterDTO = _detailedFighterHandler.HandleRequestAsync(fighter.Id).Result;
        ...
        private void SetupServices()
        {
            ...
            _mapper = new MapperConfiguration(c => { c.AddProfile<CommandModule>(); c.AddProfile<DtoModule>(); }).CreateMapper();

            _detailedFighterHandler = new DetailedFighterHandler(fighterRepository, _mapper);
            _inviteAnsweredHandler = new InviteAnsweredHandler(_mapper, inviteRepository, tournamentFighterRepository);
            _testRegisteredHandler = new TestRegisteredHandler(fighterRepository, isolationBubbleRepository, covidTestRepository, _mapper);
        }

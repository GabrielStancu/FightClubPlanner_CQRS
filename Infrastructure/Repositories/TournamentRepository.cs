        ...
        public async Task<Tournament> SelectTournamentWithDetailsByIdAsync(int id)
        {
            ...
            if (tournament != null)
            {
                ...
                foreach (var tf in tournament.TournamentFighters)
                {
                    ...
                    var recentTestHistory = new List<CovidTest>();
                    foreach (var test in fighter.TestHistory)
                    {
                        if ((DateTime.Today - test.TestDate).Days <= 21)
                        {
                            recentTestHistory.Add(test);
                        }
                    }
                    fighter.TestHistory = new List<CovidTest>(recentTestHistory);
